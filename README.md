...
> **要搬到另一台电脑？** 见 `docs/DEPLOY.md`（含最小准备清单）。打包双击 `make_package.cmd`，
> 它会自动排除 `.venv` 和数据库——`.venv` 拷过去必坏，新机器上重建就行。

---

## 一、实测结论（2026-09 实测，含一次自我纠错）
...
三套测试都是纯 Node / Python，**不需要浏览器**：

```powershell
node extension\test\ws-decode.test.mjs        # 抖音 WS 解码（16 项断言）
node extension\test\bili-decode.test.mjs      # B站 WS 解码（21 项断言）
node server\test\dashboard.test.mjs           # 面板静态自检（7 项）
.\.venv\Scripts\python.exe server\selftest.py # 服务端全链路自检
```

改协议解析或面板之后先跑一遍，能拦住大多数低级错误（本项目开发过程中，
"帧结构解错""面板引用了不存在的元素""指标写死"这几类问题都是靠它们拦下来的）。
...
| 文档 | 内容 |
|---|---|
| [`docs/DEPLOY.md`](docs/DEPLOY.md) | 换新电脑的部署清单（含最小准备清单与故障排查表） |
| [`docs/BILIBILI-PORT.md`](docs/BILIBILI-PORT.md) | B站协议设计 + **实测报告**：帧格式、被推翻的假设、指标语义、已知限制 |
